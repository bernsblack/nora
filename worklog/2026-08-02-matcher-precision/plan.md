...
- **Next:** nothing here. **This task is closed.** The four remaining failures were deliberately not fixed, and the reason is the decision below.
- **Decisions taken:**
  - **Option D, 2026-08-03: stop spending bag of words work on the last four failures.** A (leave it), B (take the copula change and lose Halina entirely) and C (score something other than a bag of words) were all on the table. D says the four are the tail of an approach that is likely to be replaced, so tuning it further is work on something that may not survive.
  - **The four stay red, as acceptance criteria rather than as a backlog.** They are the standing description of what bag of words matching cannot do, and the bar for whatever replaces the matcher. Promoted into `claude/rules/voice.md` and `claude/rules/testing.md` so a fresh session does not read a red suite as a broken build and start tuning.
  - **The matcher is a candidate for replacement by a small model doing classification, and the answer policy is not.** Classification is not generation: a model may decide the intent and the subject, and hands off to the scripted policy unchanged. Nothing about the three floors moves into a prompt. Carried into `worklog/2026-08-02-on-device-speech/`, which is now the same question as this one.
  - Correcting an earlier suggestion in this plan, which was wrong: an intent scoped threshold does **not** help. "where Stefan" and "where is Stefan" tokenise identically to `["where","stefan"]` and both score 1.000, so no threshold can separate them.
...
- [x] A constraint on how the code may change becomes a rule in `claude/rules/`. `voice.md` has the clock entry corrected (deleting the phrasings was not the fix and made it worse), `Intent.requires` documented, the three measured dead ends recorded so nobody repeats them, the four red scenarios named as deliberate, and the classification-is-not-generation line. `testing.md` has the four red scenarios and the rule that a sensitive scenario ships with its nearest neighbour. `agent-panel.md` has the calibration suppression note
- [x] A product number becomes a constant in `src/config/constants.ts`, with its reason. **None survived.** `MIN_PRECISION` was implemented and reverted, and the grief-path threshold turned out to be a no-op. The durable output here is three documented dead ends rather than a number
- [x] Anything a resident might say becomes a scenario in `personas/scenarios.ts`
- [x] A PROJECT.md principle with nothing behind it becomes a row in `docs/traceability.md`
- [x] Incidents in `errors.md` reviewed. Six logged, and the one worth acting on is that `privacy.test.ts` only covers TypeScript, carried into the speech worklog
