...
**One new finding, recorded rather than fixed.** A person nobody has written down is not a subject at all, so straight after setup "where is my husband", the sentence PROJECT.md section 6 uses as its example, is answered with the facility name and room number. Reproduced and written up as finding 13 in `personas/FINDINGS.md`. Not fixed here because it lands in subject recognition and the matcher, which carry four deliberate reds and a rule against tuning, and because whether it should answer gently or stay silent is a product decision.
...
- **Done:** all of phase 1 that is not blocked on a purchase or an account. Five items: the PWA and offline reload, the night contrast defect, Postgres exercised for real with a contract suite, the setup flow, and the device token. `pnpm run check` is 512 passing with the 4 persona scenarios that are red on purpose, `pnpm run e2e` is 54 passing with none skipped, and `pnpm run test:db` is 50 passing against a real Postgres.
- **Next:** the three blocked items, in the order their blockers can be cleared. The Vercel account and a device unblock the deploy and produce the section 5 cost number at the same time. The answer policy permission needs Bernard rather than an engineer. The upload path needs a storage vendor.
- **Then:** phase 3 is the long pole in calendar time and nothing in it has started. It is worth beginning before the three above are cleared, not after.
- **Note on the first draft.** This plan originally led with the gates and the decisions, which made a milestone that is roughly half engineering read as pure coordination. Phase 1 was eight items of code and it was the largest block of engineering since step 2.
- **Open decisions:**
  ...
  - **Who may change the answer policy.** Not who may sign in, which is a vendor question, but which family member may change the setting that decides what a person believes about whether their husband is alive. Anna and Pieter disagree about truthfulness in their own persona files, so a single shared permission means whichever of them edits last wins, silently. Today `canAccess` is the only check and that is exactly what happens.
  ...
  - **Launch market**, which the first facility decides in practice whether or not it is decided on paper.
  - ~~`MIN_INK_DIM` and which ink the night lines use~~. Settled 2026-08-03. The location line became primary ink rather than the dim floor moving, because raising the floor makes the screen brighter at three in the morning and that constant exists to prevent exactly that.
